...
def package_tag(name):
    """Generate a tag used to assicate the target with a python package.
...
def py_module(name, package, **kwargs):
    """A wrapper for `py_library` that allows association with a particular python pacakge.

    An alternative to this rule would simply be applying the following to your targets:

    ```python
    load("//tools/python/wheel:defs.bzl", "package_tag")

    PACKAGE_TAG = package_tag("my_python_package")
...
def _py_wheel_package_impl(ctx):
    ...
    out_version = ctx.outputs.out_version
    ctx.actions.write(
        output = out_version,
...
py_wheel_package = rule(
    ...
    attrs = {
        ...
        "out_version": attr.output(
            doc = "The output name of the version file.",
            mandatory = True,
        ),
        "package": attr.label(
...
def _py_wheel_destination_file_impl(ctx):
    ...
    args.add("--wheel", ctx.file.wheel)
    args.add("--wheel_name_file", wheel_info.name_file)
    args.add("--destination", ctx.attr.destination)

    ctx.actions.run(
        mnemonic = "PyWheelUrlGenerator",
        progress_message = "PyWheelUrlGenerator {}".format(ctx.attr.wheel.label),
        executable = ctx.executable._url_generator,
        arguments = [args],
        inputs = [ctx.file.wheel, wheel_info.name_file],
        outputs = [output],
    )
...
py_wheel_destination_file = rule(
    doc = "Compute the upload URL for a given wheel",
    implementation = _py_wheel_destination_file_impl,
    attrs = {
        "destination": attr.string(
            doc = (
                "The url to publish wheels to. The value passed will have the " +
                "following format strings resolved. `{WHEEL_NAME}` - the " +
                "resolved name of the wheel, `{VERSION}` - the resolved " +
        ...
        "_url_generator": attr.label(
            cfg = "exec",
            executable = True,
            default = Label("//python/wheel/private:url_generator"),
        ),
    },
)
...
py_wheel_toolchain = rule(
    doc = "A toolchain for powering the `py_wheel` rules.",
...
def py_wheel_library(
    ...
    load("//tools/python/wheel:defs.bzl", "package_tag", "py_wheel_library")
    ...
    | `{name}.publish` | A utility executable for publishing the wheel to a remote index. |
    ...
    Args:
        ...
        python_requires (str): Python versions required by this distribution, e.g. '>=3.5,<3.7'
        python_tag (str): Supported Python version(s), eg `py3`, `cp35.cp36`, etc
    ...
    version_file = "{}.version.txt".format(name)
